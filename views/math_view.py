...
class MathView:
    """Vista para el análisis matemático y el PDA con navegación enriquecida."""
    ...
    def generate_graphviz(self, current_state=None):
        ...
        for s in self.automata.transitions.keys():
            attrs = []
            if s == current_state:
                attrs.extend(['fillcolor="#ffe0b2"', 'color="#fb8c00"', "penwidth=3"])
            if s in self.automata.estados_aceptacion:
                attrs.append("shape=doublecircle")
            if s == "q_error":
                attrs.extend(["color=red", "fontcolor=red"])
            dot += f'  {s} [{", ".join(attrs)}];\n'
    ...
    def show(self):
        """Renderiza la UI de matemáticas con navegación enriquecida."""
        ...
        with col_main:
            ...
            if user_expr:
                ...
                # Reiniciar índice cuando cambia la expresión
                if (
                    "last_math_expr" not in st.session_state
                    or st.session_state.last_math_expr != user_expr
                ):
                    st.session_state.math_step  = len(history) - 1
                    st.session_state.math_slider = 0   # ← limpiar slider
                ...
                if is_valid:
                    ...
                    opc = st.radio(
                        "Operación:",
                        ["Simplificación", "Evaluación", "Resolución", "Ver Tuplas"],
                        horizontal=True,
                    )
                    ...
                    if opc == "Simplificación":
                        ...
                        vars_needed = sorted(
                            list(set(re.findall(r"[a-zA-Z][a-zA-Z0-9]*", user_expr)))
                        )
                        user_vals = {v: st.number_input(f"{v}:", value=1.0) for v in vars_needed}
                        res = self.engine.solve_symbolic(
                            user_expr, mode="evaluate", var_values=user_vals
                        )
        ...
        with col_side:
            ...
            if user_expr and "history" in locals():
                ...
                st.session_state.math_step = step_navigator(
                    current   = st.session_state.math_step,
                    max_steps = max_step,
                    key       = "math",
                )
                ...
                st.graphviz_chart(
                    self.generate_graphviz(current_step_data["Estado"])
                )
                st.code(
                    f"Carácter: {current_step_data['Carácter']}\n"
                    f"Estado:   {current_step_data['Estado']}\n"
                    f"Pila:     {current_step_data['Pila']}"
                )